...
public class MainView : MonoBehaviour
{
 ...
 // Base URL for the API
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (progressData != null)
                    {
                        ...
                        // Update lesson statuses in UnitLessonManager
                        UpdateLessonStatuses(progressData);
                    }
                    else
                    {
    ...
    // New method to log the status of each lesson
    private void LogLessonStatuses(JObject progressData)
    {
        try
        {
            JToken unitsToken = progressData["units"];
            if (unitsToken != null)
            {
                foreach (JProperty unitProp in unitsToken)
                {
                    string unitName = unitProp.Name;
                    ...
                    if (lessonsToken != null)
                    {
                        ...
                        {
                            string lessonName = lessonProp.Name;
                            string status = lessonProp.Value["status"]?.ToString() ?? "Unknown";

                            Debug.Log($"Unit: {unitName}, Lesson: {lessonName}, Status: {status}");
                        }
                    }
                }
            }
            ...
            {
                Debug.LogWarning("No units found in progress data.");
            }
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error logging lesson statuses: {ex.Message}");
        }
    }


//here
    // New method to update lesson statuses in UnitLessonManager
private void UpdateLessonStatuses(JObject progressData)
{
    try
    {
        // Find UnitLessonManager in the scene
        UnitLessonManager unitLessonManager = FindObjectOfType<UnitLessonManager>();
        if (unitLessonManager == null)
        {
            Debug.LogWarning("UnitLessonManager not found in the scene.");
            return;
        }

        // Update tutorial status
        JToken tutorialToken = progressData["tutorial"];
        if (tutorialToken != null)
        {
            string tutorialStatus = tutorialToken["status"]?.ToString() ?? "Locked";
            unitLessonManager.UpdateLessonStatusIndicator("Tutorial", "Tutorial", tutorialStatus);
        }

        // Iterate through units and lessons in the progress data
        JToken unitsToken = progressData["units"];
        if (unitsToken != null)
        {
            foreach (JProperty unitProp in unitsToken)
            {
                ...
                // Update post-test status for Unit 1 and Unit 2
                if (unitName == "Unit1" || unitName == "Unit2")
                {
                    JToken postTestToken = unitValue["postTest"];
                    if (postTestToken != null)
                    {
                        string postTestStatus = postTestToken["status"]?.ToString() ?? "Locked";
                        unitLessonManager.UpdateLessonStatusIndicator(unitName, "PostTest", postTestStatus);
                    }
                }
                ...
                if (lessonsToken != null)
                {
                    foreach (JProperty lessonProp in lessonsToken)
                    {
                        ...
                        string status = lessonValue["status"]?.ToString() ?? "Locked";

                        // Update the status indicator text in UnitLessonManager
                        unitLessonManager.UpdateLessonStatusIndicator(unitName, lessonName, status);
                    }
                }
            }
        }
    ...
    {
        Debug.LogError($"Error updating lesson statuses: {ex.Message}");
    }
}
    //end here
