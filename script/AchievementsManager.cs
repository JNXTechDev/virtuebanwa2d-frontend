  ...
  [SerializeField]  private string baseUrl => NetworkConfig.BaseUrl;
   //  [SerializeField] private  string baseUrl => NetworkConfig.BaseUrl + "/api"; 
    ...
    private void OnEnable()
    {
        ...
        // Clear cache when panel is enabled to force a fresh load
        dialogueCache.Clear();
    ...
    private async void FetchAndDisplayAchievements()
    {
        try
        {
            // Add a small delay to ensure any previous async operations have completed
            await Task.Delay(10);
            ...
            {
                // Keep track of uniquely created entries to prevent duplicates
                HashSet<string> displayedAchievements = new HashSet<string>();
                ...
                {
                    // Skip if we've already displayed this achievement
                    if (displayedAchievements.Contains(achievement.UniqueId))
                    {
                        Debug.Log($"Skipping duplicate tutorial achievement: {achievement.Title}");
                        continue;
                    }
                    ...
                    {
                        createdEntries++;
                        displayedAchievements.Add(achievement.UniqueId);
                        Debug.Log($"Added tutorial achievement: {achievement.Title} with ID {achievement.UniqueId}");
                    }
                    await Task.Delay(30);
                }
                ...
                foreach (var achievement in lessonAchievements)
                {
                    ...
                    if (displayedAchievements.Contains(achievement.UniqueId))
                    {
                        Debug.Log($"Skipping duplicate lesson achievement: {achievement.Title}");
                        continue;
                    }
                    ...
                    if (success)
                    {
                        createdEntries++;
                        displayedAchievements.Add(achievement.UniqueId);
                        Debug.Log($"Added lesson achievement: {achievement.Title} with ID {achievement.UniqueId}");
                    }
                    await Task.Delay(30);
                }
                ...
                // If we didn't create any entries despite having achievements, show the no achievements text
                if (createdEntries == 0 && noAchievementsText != null)
                {
                    noAchievementsText.gameObject.SetActive(true);
                    noAchievementsText.text = "Could not display achievements. Please try again later.";
                }
            }
        }
        catch (Exception ex)
    ...
    private async Task<List<AchievementData>> GetPlayerAchievements(string username)
    {
        ...
        try
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                // Add cache-busting parameter to avoid browser/Unity cache
                string cacheKey = DateTime.Now.Ticks.ToString();
                url = $"{url}?cache={cacheKey}";
                ...
                {
                    // Check the main tutorial reward only if it's explicitly present and not empty
                    if (tutorial["reward"] != null && !string.IsNullOrEmpty(tutorial["reward"].ToString()))
                    {
                        // Only show complete tutorial if it's marked as "Completed"
                        if (tutorial["status"] != null && tutorial["status"].ToString() == "Completed")
                        {
                    ...
                    // Process tutorial checkpoints - these are individual NPC achievements
                    if (tutorial["checkpoints"] is JObject checkpoints)
                    {
                        ...
                        foreach (var checkpoint in checkpoints)
                        {
                            ...
                            if (checkpointData != null && 
                                ...
                                checkpointData["status"] != null && 
                                checkpointData["status"].ToString() == "Completed")
                            {
                                // Create a unique key for this achievement that includes all important details
                                ...
                                if (!addedAchievements.Contains(achievementKey))
                                {
                                    ...
                                    achievements.Add(new AchievementData
                                    {
                                        ...
                                        NpcName = npcName,
                                        // Add a unique identifier to help with duplicate detection
                                        UniqueId = achievementKey
                                    });
                                    achievementCount++;
                                    addedAchievements.Add(achievementKey);
                                ...
                                {
                                    Debug.Log($"Skipped duplicate achievement for {npcName}");
                                }
                            }
                        }
                    }
                        ...
                        if (unitData != null && unitData["lessons"] is JObject lessons)
                        {
                            foreach (var lesson in lessons)
                            {
                                ...
                                if (lessonData != null && 
                                    ...
                                    lessonData["status"]?.ToString() == "Completed")
                                {
                ...
                // Additional de-duplication: Identify duplicate achievements based on their key data
                Dictionary<string, AchievementData> uniqueAchievements = new Dictionary<string, AchievementData>();
                
                foreach (var achievement in achievements)
                {
                    if (!string.IsNullOrEmpty(achievement.UniqueId))
                    {
                        // Only add if it doesn't exist or if the existing one is older
                        if (!uniqueAchievements.ContainsKey(achievement.UniqueId) || 
                            uniqueAchievements[achievement.UniqueId].Date < achievement.Date)
                        {
                            uniqueAchievements[achievement.UniqueId] = achievement;
                        }
                    }
                }
                
                // Replace the list with the deduplicated dictionary values
                achievements = uniqueAchievements.Values.ToList();
                
                Debug.Log($"Final achievement count after de-duplication: {achievements.Count}");
            }
        }
        catch (Exception ex)
        {
    ...
    private class AchievementData
    {
        ...
        public string UniqueId { get; set; } = ""; // Added field to help with duplicate detection
    }
}
